...
CMA <- function(cidade, legenda, mintempo,maxtempo){
  ...
  mapa_renda <- ggplot()+
    ...
    ggspatial::annotation_scale(location = "br", width_hint = 0.2, pad_y = unit(0, "cm")) +
    coord_sf(datum=NA) +
    labs(title = "Acessibilidade",
         fill = "Jobs accessible\n(x10.000)") +
    if(legenda == "sim"){
...
fig5 <- ggdraw(xlim = c(0,135), ylim = c(0,165))+
          ...
          draw_label("S?o Paulo", size = 18, fontface = "bold", x=97, y=35,angle = 270,fontfamily = "serif")+
...
ggsave2(fig5, filename="./figures/Fig5_English.png",
